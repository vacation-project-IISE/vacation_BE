...
exports.addToWishlist = async (req, res) => {
  const { user_id, product_name, price, category_name, image_url, image_alt } =
    req.body;
  ...
  try {
    ...
    return res.status(200).json({ message: "위시리스트에 추가되었습니다." });
  } catch (error) {
...
exports.getWishlist = async (req, res) => {
  ...
  try {
    ...
    if (querySnapshot.empty) {
      return res.status(404).json({ message: "위시리스트가 비어있습니다." });
    }
...
exports.removeFromWishlist = async (req, res) => {
  ...
  try {
    ...
    return res.status(200).json({ message: "위시리스트에서 삭제되었습니다." });
  } catch (error) {
